...
public class MovieListing
{
    ...
    public static void createMovies(){
		...
		System.out.println("after create weeklyshowin:::::movie size" + moviesPlaying.size());
    }

    
    @SuppressWarnings("deprecation")
	public static void createWeeklyShowings(int i)
    {
    		...
    		for(int j=0;j<currentShowings.size();j++)
    		{
    			if((!currentShowings.get(j).isCopy())&&(!currentShowings.get(j).hasCreatedWeeklyShowings()))
    			{
    				if(moviesPlaying.get(i).getShowings().get(j).isWeekly() && moviesPlaying.get(i).getShowings().get(j).getCineplex().getForwardScheduling() >=7)
        			{
        				...
        				System.out.println(new Date(moviesPlaying.get(i).getShowings().get(j).getDate().getTime()));
    ...
    public static void addShowing(Movie movie)
    {
